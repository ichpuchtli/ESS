...
void QCPPainter::setAntialiasing( bool enabled )
{
  ...
  if ( mIsAntialiasing != enabled ) {
    ...
    if ( !mModes.testFlag(
           pmVectorized ) ) { // antialiasing half-pixel shift only needed for rasterized outputs
      if ( mIsAntialiasing )
        translate( 0.5, 0.5 );
      else
        translate( -0.5, -0.5 );
...
{
  return mVisible && ( !mParentLayerable
...
void QCPLayoutElement::update()
{
  if ( mAutoMargins != QCP::msNone ) {
    ...
    QVector<QCP::MarginSide> marginSides = QVector<QCP::MarginSide>() << QCP::msLeft
    ...
    QVector<int> minSizes, QVector<double> stretchFactors, int totalSize ) const
{
  ...
  while ( !unfinishedSections.isEmpty()
          ...
          2 ) { // the iteration check ist just a failsafe in case something really strange happens
    ++outerIterations;
    ...
    while ( !unfinishedSections.isEmpty()
      ...
      if ( nextMax <
        ...
        unfinishedSections.removeOne(
          nextId ); // exclude the section that is now at maximum from further changes
      ...
      for ( int i = 0; i < sectionCount; ++i ) {
        if ( !minimumLockedSections.contains(
          ...
          freeSize -= sectionSizes.at(
                        i ); // remove size of minimum locked sections from available space in next round
      }
...
void QCPLayoutGrid::insertRow( int newIndex )
{
  if ( mElements.isEmpty()
       || mElements.first().isEmpty() ) { // if grid is completely empty, add first cell
    expandTo( 1, 1 );
    return;
  }
...
QSize QCPLayoutGrid::maximumSizeHint() const
{
  ...
  for ( int i = 0; i < maxColWidths.size(); ++i )
    result.setWidth( qMin( result.width() + maxColWidths.at( i ),
                           QWIDGETSIZE_MAX ) );
...
void QCPLayoutInset::updateLayout()
{
  for ( int i = 0; i < mElements.size(); ++i ) {
    ...
    finalMaxSize.setWidth( mElements.at( i )->maximumSize().width() <
                           QWIDGETSIZE_MAX ? mElements.at( i )->maximumSize().width() :
                           maxSizeHint.width() );
...
double QCPLineEnding::boundingDistance() const
{
  ...
  case esSkewedBar:
    return qSqrt( mWidth * mWidth + mLength *
                  mLength ); // items that have width and length
                          ...
                          const QVector2D &dir ) const
{
  ...
  case esSkewedBar: {
    if ( qFuzzyIsNull( painter->pen().widthF() )
      ...
      painter->drawLine( ( pos + widthVec + lengthVec * 0.2 *
      ...
      painter->drawLine( ( pos + widthVec + lengthVec * 0.2 *
                           ( mInverted ? -1 : 1 ) + dir.normalized()*qMax( 1.0,
                               ( double )painter->pen().widthF() ) * 0.5 ).toPointF(),
                         ( pos - widthVec - lengthVec * 0.2 * ( mInverted ? -1 : 1 ) + dir.normalized()
                           *qMax( 1.0, ( double )painter->pen().widthF() ) * 0.5 ).toPointF() );
    }
    break;
  }
  }
...
QCPAxis::QCPAxis( QCPAxisRect *parent, AxisType type ) :
  ...
  mOffset( 0 ),
  mPadding( 5 ),
  mOrientation( ( type == atBottom
                  || type == atTop ) ? Qt::Horizontal : Qt::Vertical ),
...
void QCPAxis::setScaleLogBase( double base )
{
  if ( base > 1 ) {
    ...
    mScaleLogBaseLogInv = 1.0 / qLn(
                            mScaleLogBase ); // buffer for faster baseLog() calculation
    mCachedMarginValid = false;
  } else
...
void QCPAxis::setSelectedParts( const SelectableParts &selected )
{
  if ( mSelectedParts != selected ) {
    if ( mSelectedParts.testFlag( spTickLabels ) != selected.testFlag(
           spTickLabels ) )
      mLabelCache.clear();
...
void QCPAxis::setNumberFormat( const QString &formatCode )
{
  ...
  if ( formatCode.at( 1 ) == 'b' && ( mNumberFormatChar == 'e'
                                      || mNumberFormatChar == 'g' ) ) {
    mNumberBeautifulPowers = true;
  } else {
    qDebug() << Q_FUNC_INFO <<
...
double QCPAxis::pixelToCoord( double value ) const
{
  if ( orientation() == Qt::Horizontal ) {
    if ( mScaleType == stLinear ) {
      ...
      if ( !mRangeReversed )
        return pow( mRange.upper / mRange.lower,
    ...
    if ( mScaleType == stLinear ) {
      ...
      if ( !mRangeReversed )
        return pow( mRange.upper / mRange.lower,
                    ( mAxisRect->bottom() - value ) / ( double )mAxisRect->height() ) *
               mRange.lower;
      else
        return pow( mRange.upper / mRange.lower,
                    ( value - mAxisRect->bottom() ) / ( double )mAxisRect->height() ) *
               mRange.upper;
    }
  }
}
...
double QCPAxis::coordToPixel( double value ) const
{
  if ( orientation() == Qt::Horizontal ) {
    if ( mScaleType == stLinear ) {
      ...
      if ( value >= 0
           && mRange.upper <
           0 ) // invalid value for logarithmic scale, just draw it outside visible range
                            ...
                            QVariant *details ) const
{
  ...
  if ( ( onlySelectable && !mSelectableParts.testFlag( part ) )
       || part == spNone )
    return -1;
...
void QCPAxis::setupTickVectors()
{
  ...
  mExponentialChar =
    mParentPlot->locale().exponential();   // will be needed when drawing the numbers generated here, in getTickLabelData()
  mPositiveSignChar =
    mParentPlot->locale().positiveSign(); // will be needed when drawing the numbers generated here, in getTickLabelData()
  if ( mAutoTickLabels ) {
    ...
    if ( mTickLabelType == ltNumber ) {
      for ( int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i )
        mTickVectorLabels[i] = mParentPlot->locale().toString( mTickVector.at( i ),
                               mNumberFormatChar, mNumberPrecision );
      ...
      for ( int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i ) {
        ...
        mTickVectorLabels[i] = mParentPlot->locale().toString( QDateTime::fromTime_t(
                                 mTickVector.at( i ) ), mDateTimeFormat );
#else
        mTickVectorLabels[i] = mParentPlot->locale().toString(
                                 QDateTime::fromMSecsSinceEpoch( mTickVector.at( i ) * 1000 ), mDateTimeFormat );
#endif
      }
    }
    ...
    if ( mAutoTickStep ) {
      ...
      double magnitudeFactor = qPow( 10.0,
                                     qFloor( qLn( mTickStep ) / qLn(
                                         10.0 ) ) ); // get magnitude factor e.g. 0.01, 1, 10, 1000 etc.
...
{
  int result =
    mSubTickCount; // default to current setting, if no proper value can be found
...
void QCPAxis::draw( QCPPainter *painter )
{
  ...
  double xCor = 0, yCor =
                  0; // paint system correction, for pixel exact matches (affects baselines and ticks of top/right axes)
  switch ( mAxisType ) {
  ...
  if ( orientation() == Qt::Horizontal )
    baseLine.setPoints( origin + QPointF( xCor, yCor ),
                        origin + QPointF( mAxisRect->width() + xCor, yCor ) );
  else
  ...
  if ( mTicks ) {
    ...
    if ( orientation() == Qt::Horizontal ) {
      for ( int i = lowTick; i <= highTick; ++i ) {
        ...
        painter->drawLine( QLineF( t + xCor,
                                   origin.y() - mTickLengthOut * tickDir + yCor, t + xCor,
                                   origin.y() + mTickLengthIn * tickDir + yCor ) );
      }
    } else {
      for ( int i = lowTick; i <= highTick; ++i ) {
  ...
  if ( mTicks && mSubTickCount > 0 ) {
    ...
    int tickDir = ( mAxisType == atBottom || mAxisType == atRight ) ? -1 : 1;
    if ( orientation() == Qt::Horizontal ) {
      for ( int i = 0; i < mSubTickVector.size();
            ++i ) { // no need to check bounds because subticks are always only created inside current mRange
        t = coordToPixel( mSubTickVector.at( i ) );
        painter->drawLine( QLineF( t + xCor,
                                   origin.y() - mSubTickLengthOut * tickDir + yCor, t + xCor,
                                   origin.y() + mSubTickLengthIn * tickDir + yCor ) );
      }
    } else {
      for ( int i = 0; i < mSubTickVector.size(); ++i ) {
        t = coordToPixel( mSubTickVector.at( i ) );
        painter->drawLine( QLineF( origin.x() - mSubTickLengthOut * tickDir + xCor,
                                   t + yCor, origin.x() + mSubTickLengthIn * tickDir + xCor, t + yCor ) );
      }
    }
  }
  ...
  painter->setAntialiasing(
    true ); // always want endings to be antialiased, even if base and ticks themselves aren't
  ...
  if ( mLowerEnding.style() != QCPLineEnding::esNone )
    mLowerEnding.draw( painter,
                       QVector2D( baseLine.p1() ) - baseLineVector.normalized()
                       *mLowerEnding.realLength() * ( mLowerEnding.inverted() ? -1 : 1 ),
                       -baseLineVector );
  if ( mUpperEnding.style() != QCPLineEnding::esNone )
    mUpperEnding.draw( painter,
                       QVector2D( baseLine.p2() ) + baseLineVector.normalized()
                       *mUpperEnding.realLength() * ( mUpperEnding.inverted() ? -1 : 1 ),
                       baseLineVector );
  ...
  if ( mTickLabels ) {
    ...
    for ( int i = lowTick; i <= highTick; ++i ) {
      ...
      placeTickLabel( painter, t, margin, mTickVectorLabels.at( i ),
                      &tickLabelsSize );
    }
  }
  if ( orientation() == Qt::Horizontal )
  ...
  if ( !mLabel.isEmpty() ) {
    ...
    if ( mAxisType == atLeft ) {
      ...
      painter->drawText( origin.x(), origin.y() - margin - labelBounds.height(),
                         mAxisRect->width(), labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter,
                         mLabel );
    else if ( mAxisType == atBottom )
      painter->drawText( origin.x(), origin.y() + margin, mAxisRect->width(),
                         labelBounds.height(), Qt::TextDontClip | Qt::AlignCenter, mLabel );
  }

  // set selection boxes:
  int selAxisOutSize = qMax( qMax( mTickLengthOut, mSubTickLengthOut ),
                             mParentPlot->selectionTolerance() );
  int selAxisInSize = mParentPlot->selectionTolerance();
  int selTickLabelSize = ( orientation() == Qt::Horizontal ?
                           tickLabelsSize.height() : tickLabelsSize.width() );
  int selTickLabelOffset = qMax( mTickLengthOut,
                                 mSubTickLengthOut ) + mTickLabelPadding;
  int selLabelSize = labelBounds.height();
  int selLabelOffset = selTickLabelOffset + selTickLabelSize + mLabelPadding;
  if ( mAxisType == atLeft ) {
    mAxisSelectionBox.setCoords( origin.x() - selAxisOutSize, mAxisRect->top(),
                                 origin.x() + selAxisInSize, mAxisRect->bottom() );
    ...
    mLabelSelectionBox.setCoords( origin.x() - selLabelOffset - selLabelSize,
                                  mAxisRect->top(), origin.x() - selLabelOffset, mAxisRect->bottom() );
  } else if ( mAxisType == atRight ) {
    mAxisSelectionBox.setCoords( origin.x() - selAxisInSize, mAxisRect->top(),
                                 origin.x() + selAxisOutSize, mAxisRect->bottom() );
    mTickLabelsSelectionBox.setCoords( origin.x() + selTickLabelOffset +
                                       selTickLabelSize, mAxisRect->top(), origin.x() + selTickLabelOffset,
                                       mAxisRect->bottom() );
    mLabelSelectionBox.setCoords( origin.x() + selLabelOffset + selLabelSize,
    ...
    mAxisSelectionBox.setCoords( mAxisRect->left(), origin.y() - selAxisOutSize,
                                 mAxisRect->right(), origin.y() + selAxisInSize );
    mTickLabelsSelectionBox.setCoords( mAxisRect->left(),
                                       origin.y() - selTickLabelOffset - selTickLabelSize, mAxisRect->right(),
                                       origin.y() - selTickLabelOffset );
    mLabelSelectionBox.setCoords( mAxisRect->left(),
                                  origin.y() - selLabelOffset - selLabelSize, mAxisRect->right(),
                                  origin.y() - selLabelOffset );
  } else if ( mAxisType == atBottom ) {
    mAxisSelectionBox.setCoords( mAxisRect->left(), origin.y() - selAxisInSize,
                                 mAxisRect->right(), origin.y() + selAxisOutSize );
    mTickLabelsSelectionBox.setCoords( mAxisRect->left(),
                                       origin.y() + selTickLabelOffset + selTickLabelSize, mAxisRect->right(),
                                       origin.y() + selTickLabelOffset );
    mLabelSelectionBox.setCoords( mAxisRect->left(),
...
/*! \internal
  ...
  getTickLabelData).
*/
void QCPAxis::placeTickLabel( QCPPainter *painter, double position,
                              int distanceToAxis, const QString &text, QSize *tickLabelsSize )
{
  ...
  QPointF labelAnchor;
  switch ( mAxisType ) {
  case atLeft:   labelAnchor = QPointF( mAxisRect->left() - distanceToAxis -
                                          mOffset, position ); break;
  case atRight:  labelAnchor = QPointF( mAxisRect->right() + distanceToAxis +
                                          mOffset, position ); break;
  case atTop:    labelAnchor = QPointF( position,
                                          mAxisRect->top() - distanceToAxis - mOffset ); break;
  case atBottom: labelAnchor = QPointF( position,
                                          mAxisRect->bottom() + distanceToAxis + mOffset ); break;
  }
  if ( parentPlot()->plottingHints().testFlag( QCP::phCacheLabels )
    ...
    if ( orientation() == Qt::Horizontal ) {
      if ( labelAnchor.x() + cachedLabel->offset.x() + cachedLabel->pixmap.width() >
           mParentPlot->viewport().right() ||
           labelAnchor.x() + cachedLabel->offset.x() < mParentPlot->viewport().left() )
        return;
    } else {
      if ( labelAnchor.y() + cachedLabel->offset.y() + cachedLabel->pixmap.height() >
           mParentPlot->viewport().bottom() ||
    ...
    // if label would be partly clipped by widget border on sides, don't draw it:
    if ( orientation() == Qt::Horizontal ) {
      if ( finalPosition.x() + ( labelData.rotatedTotalBounds.width() +
                                 labelData.rotatedTotalBounds.left() ) > mParentPlot->viewport().right() ||
           finalPosition.x() + labelData.rotatedTotalBounds.left() <
           mParentPlot->viewport().left() )
        return;
    } else {
      if ( finalPosition.y() + ( labelData.rotatedTotalBounds.height() +
                                 labelData.rotatedTotalBounds.top() ) > mParentPlot->viewport().bottom() ||
           finalPosition.y() + labelData.rotatedTotalBounds.top() <
           mParentPlot->viewport().top() )
        return;
    }
...
*/
void QCPAxis::drawTickLabel( QCPPainter *painter, double x, double y,
                             const QCPAxis::TickLabelData &labelData ) const
{
    ...
    const QString &text ) const
{
  ...
  if ( useBeautifulPowers ) {
    ...
    if ( mScaleType == stLogarithmic && result.basePart == "1" )
      ...
      result.basePart += ( mNumberMultiplyCross ? QString( QChar( 215 ) ) : QString(
                             QChar( 183 ) ) ) + "10";
    result.expPart = text.mid( ePos + 1 );
    // clip "+" and leading zeros off expPart:
    while ( result.expPart.at( 1 ) == '0'
            && result.expPart.length() >
            2 ) // length > 2 so we leave one zero when numberFormatChar is 'e'
...
*/
QPointF QCPAxis::getTickLabelDrawOffset( const QCPAxis::TickLabelData
    &labelData ) const
{
  /*
  ...
  */
  ...
  bool flip = qFuzzyCompare( qAbs( mTickLabelRotation ),
...
*/
void QCPAxis::getMaxTickLabelSize( const QFont &font, const QString &text,
...
int QCPAxis::calculateMargin()
{
  ...
  if ( mVisible ) {
    ...
    if ( mTickLabels ) {
      for ( int i = lowTick; i <= highTick; ++i )
        getMaxTickLabelSize( mTickLabelFont, mTickVectorLabels.at( i ),
                             &tickLabelsSize ); // don't use getTickLabelFont() because we don't want margin to possibly change on selection
    ...
    if ( !mLabel.isEmpty() ) {
      QFontMetrics fontMetrics(
        mLabelFont ); // don't use getLabelFont() because we don't want margin to possibly change on selection
      QRect bounds;
      bounds = fontMetrics.boundingRect( 0, 0, 0, 0,
                                         Qt::TextDontClip | Qt::AlignHCenter | Qt::AlignVCenter, mLabel );
...
bool QCPAbstractPlottable::removeFromLegend() const
{
  ...
  if ( QCPPlottableLegendItem *lip = mParentPlot->legend->itemWithPlottable(
                                       this ) )
    ...
    double value ) const
{
  ...
  if ( keyAxis->orientation() == Qt::Horizontal )
    return QPointF( keyAxis->coordToPixel( key ),
                    valueAxis->coordToPixel( value ) );
  else
    return QPointF( valueAxis->coordToPixel( value ),
                    keyAxis->coordToPixel( key ) );
}
...
QCPItemAnchor::~QCPItemAnchor()
{
  ...
  for ( int i = 0; i < currentChildren.size(); ++i )
    currentChildren.at( i )->setParentAnchor(
      0 ); // this acts back on this anchor and child removes itself from mChildren
}
...
void QCPItemPosition::setType( QCPItemPosition::PositionType type )
{
  if ( mPositionType != type ) {
    ...
    if ( ( mPositionType == ptPlotCoords || type == ptPlotCoords ) && ( !mKeyAxis
         || !mValueAxis ) )
      recoverPixelPosition = false;
    if ( ( mPositionType == ptAxisRectRatio || type == ptAxisRectRatio )
         && ( !mAxisRect ) )
      recoverPixelPosition = false;
    ...
    notAntialiasedElement, bool enabled )
{
  if ( !enabled && mNotAntialiasedElements.testFlag( notAntialiasedElement ) )
    mNotAntialiasedElements &= ~notAntialiasedElement;
            ...
            && !mNotAntialiasedElements.testFlag( notAntialiasedElement ) )
    mNotAntialiasedElements |= notAntialiasedElement;
    ...
    bool onlySelectable ) const
{
  ...
  for ( int i = 0; i < mPlottables.size(); ++i ) {
    ...
    if ( onlySelectable
      ...
      continue;
    if ( ( currentPlottable->keyAxis()->axisRect()->rect() &
           currentPlottable->valueAxis()->axisRect()->rect() ).contains(
...
QCPLayoutElement *QCustomPlot::layoutElementAt( const QPointF &pos ) const
{
  ...
  while ( searchSubElements && current ) {
    ...
    for ( int i = 0; i < elements.size(); ++i ) {
      if ( elements.at( i ) && elements.at( i )->realVisibility()
           && elements.at( i )->selectTest( pos, false ) >= 0 ) {
...
void QCustomPlot::rescaleAxes( bool onlyVisible )
{
  ...
  for ( int i = 0; i < mPlottables.size(); ++i ) {
    if ( mPlottables.at( i )->realVisibility() || !onlyVisible ) {
      mPlottables.at( i )->rescaleAxes(
        !firstPlottable ); // onlyEnlarge disabled on first plottable
...
void QCustomPlot::mousePressEvent( QMouseEvent *event )
{
  ...
  mMousePressPos =
    event->pos(); // need this to determine in releaseEvent whether it was a click (no position change between press and release)
...
void QCustomPlot::mouseReleaseEvent( QMouseEvent *event )
{
  ...
  if ( ( mMousePressPos - event->pos() ).manhattanLength() <
    ...
    QCPLayerable *clickedLayerable = layerableAt( event->pos(), false,
                                     &details ); // for these signals, selectability is ignored, that's why we call this again with onlySelectable set to false
    if ( QCPAbstractPlottable *ap = qobject_cast<QCPAbstractPlottable*>
                                    ( clickedLayerable ) )
...
void QCPGraph::getPlotData( QVector<QPointF> *lineData,
                            QVector<QCPData> *pointData ) const
{
  switch ( mLineStyle ) {
                                    ...
                                    QVector<QCPData> *pointData ) const
{
  ...
  if ( dataCount > 0 ) {
    lineData->reserve( dataCount * 2 +
                       2 ); // added 2 to reserve memory for lower/upper fill base points that might be needed for fill
    lineData->resize( dataCount *
                      2 ); // multiplied by 2 because step plot needs two polyline points per one actual data point
    if ( pointData )
      pointData->resize( dataCount );
                                   ...
                                   QVector<QCPData> *pointData ) const
{
  ...
  if ( dataCount > 0 ) {
    lineData->resize( dataCount *
                      2 ); // no need to reserve 2 extra points, because there is no fill for impulse plot
    if ( pointData )
      pointData->resize( dataCount );
  ...
  if ( mErrorType != etNone ) {
    ...
    if ( keyAxis->orientation() == Qt::Vertical ) {
      for ( int i = 0; i < pointData->size(); ++i )
        drawError( painter, valueAxis->coordToPixel( pointData->at( i ).value ),
                   keyAxis->coordToPixel( pointData->at( i ).key ), pointData->at( i ) );
    } else {
      for ( int i = 0; i < pointData->size(); ++i )
        drawError( painter, keyAxis->coordToPixel( pointData->at( i ).key ),
                   valueAxis->coordToPixel( pointData->at( i ).value ), pointData->at( i ) );
    }
  }
  ...
  if ( keyAxis->orientation() == Qt::Vertical ) {
    for ( int i = 0; i < pointData->size(); ++i )
      mScatterStyle.drawShape( painter,
                               valueAxis->coordToPixel( pointData->at( i ).value ),
                               keyAxis->coordToPixel( pointData->at( i ).key ) );
  } else {
    for ( int i = 0; i < pointData->size(); ++i )
      mScatterStyle.drawShape( painter,
                               keyAxis->coordToPixel( pointData->at( i ).key ),
                          ...
                          const QCPData &data ) const
{
  ...
  double skipSymbolMargin =
    mScatterStyle.size(); // pixels left blank per side, when mErrorBarSkipSymbol is true
  ...
  if ( keyAxis->orientation() == Qt::Vertical ) {
    ...
    if ( mErrorType == etKey || mErrorType == etBoth ) {
      ...
      if ( mErrorBarSkipSymbol ) {
        if ( a - y >
             skipSymbolMargin ) // don't draw spine if error is so small it's within skipSymbolmargin
...
void QCPGraph::getVisibleDataBounds( QCPDataMap::const_iterator &lower,
                                     QCPDataMap::const_iterator &upper, int &count ) const
{
  if ( !mKeyAxis ) {
    qDebug() << Q_FUNC_INFO << "invalid key axis";
    return;
  ...
  bool lowoutlier = lbound !=
                    mData->constBegin(); // indicates whether there exist points below axis range
  bool highoutlier = ubound !=
                     mData->constEnd(); // indicates whether there exist points above axis range

  lower = ( lowoutlier ? lbound - 1 :
            lbound ); // data point range that will be actually drawn
  upper = ( highoutlier ? ubound : ubound -
            1 ); // data point range that will be actually drawn
    ...
    *lineData ) const
{
  ...
  if ( keyAxis->orientation() == Qt::Horizontal ) {
    ...
    if ( croppedData->size() < 2 ) return
        QPolygonF(); // need at least two points for interpolation
    double slope;
...
{
  if ( mData->isEmpty() ) {
    qDebug() << Q_FUNC_INFO << "requested point distance on graph" << mName <<
             "without data";
    return 500;
  }
  if ( mData->size() == 1 ) {
  ...
  if ( mLineStyle == lsNone ) {
    ...
    double minDistSqr = std::numeric_limits<double>::max();
    QPointF ptA;
    QPointF ptB = coordsToPixels( pointData->at( 0 ).key,
                                  pointData->at(
                                    0 ).value ); // getScatterPlotData returns in plot coordinates, so transform to pixels
...
*/
QCPRange QCPGraph::getKeyRange( bool &validRange, SignDomain inSignDomain,
...
*/
QCPRange QCPGraph::getValueRange( bool &validRange, SignDomain inSignDomain,
                                  bool includeErrors ) const
{
  QCPRange range;
  bool haveLower = false;
...
void QCPCurve::getCurveData( QVector<QPointF> *lineData ) const
{
  ...
  */
  ...
  bool firstPoint =
    true; // first point must always be drawn, to make sure fill works correctly
  for ( it = mData->constBegin(); it != mData->constEnd(); ++it ) {
    x = it.value().key;
    y = it.value().value;
    ...
    */
    ...
    if ( currentRegion == 5 || ( firstPoint
                                 && mBrush.style() !=
                                 Qt::NoBrush ) ) { // current is in R, add current and last if it wasn't added already
      if ( !addedLastAlready ) // in case curve just entered R, make sure the last point outside R is also drawn correctly
        lineData->append( coordsToPixels( ( it - 1 ).value().key,
                                          ( it - 1 ).value().value ) ); // add last point to vector
      else if ( lastRegion !=
                5 ) { // added last already. If that's the case, we probably added it at optimized position. So go back and make sure it's at original position (else the angle changes under which this segment enters R)
        if ( !firstPoint ) // because on firstPoint, currentRegion is 5 and addedLastAlready is true, although there is no last point
          lineData->replace( lineData->size() - 1, coordsToPixels( ( it - 1 ).value().key,
                             ( it - 1 ).value().value ) );
      }
      lineData->append( coordsToPixels( it.value().key,
                                        it.value().value ) ); // add current point to vector
      addedLastAlready = true; // so in next iteration, we don't add this point twice
    } else if ( currentRegion !=
                lastRegion ) { // changed region, add current and last if not added already
      ...
      if ( lastRegion == 5 || // coming from R
           ( ( lastRegion == 2 && currentRegion == 4 ) || ( lastRegion == 4
               && currentRegion == 2 ) ) || // skip top left diagonal
           ( ( lastRegion == 4 && currentRegion == 8 ) || ( lastRegion == 8
               && currentRegion == 4 ) ) || // skip top right diagonal
           ( ( lastRegion == 8 && currentRegion == 6 ) || ( lastRegion == 6
               && currentRegion == 8 ) ) || // skip bottom right diagonal
           ( ( lastRegion == 6 && currentRegion == 2 ) || ( lastRegion == 2
               && currentRegion == 6 ) ) // skip bottom left diagonal
         ) {
        ...
        if ( !addedLastAlready )
          lineData->append( outsideCoordsToPixels( ( it - 1 ).value().key,
                            ( it - 1 ).value().value, currentRegion, axisRect ) );
        // add current point, optimized:
        lineData->append( outsideCoordsToPixels( it.value().key, it.value().value,
                          currentRegion, axisRect ) );
      }
      addedLastAlready =
        true; // so that if next point enters 5, or crosses another region boundary, we don't add this point twice
  ...
  if ( lastRegion != 5 && mBrush.style() != Qt::NoBrush && !mData->isEmpty() )
    lineData->append( coordsToPixels( ( mData->constEnd() - 1 ).value().key,
                                      ( mData->constEnd() - 1 ).value().value ) );
}
...
*/
QPointF QCPCurve::outsideCoordsToPixels( double key, double value, int region,
...
QCPRange QCPBars::getKeyRange( bool &validRange, SignDomain inSignDomain ) const
{
  ...
  while ( it != mData->constEnd() ) {
    ...
    if ( inSignDomain == sdBoth || ( inSignDomain == sdNegative
                                     && current + barWidthHalf < 0 ) || ( inSignDomain == sdPositive
                                         && current - barWidthHalf > 0 ) ) {
                                 ...
                                 SignDomain inSignDomain ) const
{
  QCPRange range;
  bool haveLower =
    true; // set to true, because 0 should always be visible in bar charts
  bool haveUpper =
    true; // set to true, because 0 should always be visible in bar charts
  ...
  QCPBarDataMap::const_iterator it = mData->constBegin();
  while ( it != mData->constEnd() ) {
    current = it.value().value + getBaseValue( it.value().key,
              it.value().value >= 0 );
    if ( inSignDomain == sdBoth || ( inSignDomain == sdNegative && current < 0 )
         || ( inSignDomain == sdPositive && current > 0 ) ) {
      if ( current < range.lower || !haveLower ) {
    ...
    SignDomain inSignDomain ) const
{
  if ( inSignDomain == sdBoth ) {
    ...
    QVector<double>
    values; // values that must be considered (i.e. all outliers and the five box-parameters)
    ...
    if ( pointVectors.size() == 2 ) {
      ...
      for ( int i = 0; i < pointVectors.size() - 1; ++i ) {
        for ( int k = i + 1; k < pointVectors.size(); ++k ) {
          double distSqr = ( pointVectors.at( i ) - pointVectors.at(
                               k ) ).lengthSquared();
...
void QCPItemCurve::draw( QCPPainter *painter )
{
  ...
  if ( QVector2D( endVec - startVec ).length() >
       1e10 ) // too large curves cause crash
    return;
...
void QCPItemEllipse::draw( QCPPainter *painter )
{
  ...
  if ( ellipseRect.intersects(
         clip ) ) { // only draw if bounding rect of ellipse is visible in cliprect
    painter->setPen( mainPen() );
    painter->setBrush( mainBrush() );
#ifdef __EXCEPTIONS
...
QPointF QCPItemEllipse::anchorPixelPoint( int anchorId ) const
{
  QRectF rect = QRectF( topLeft->pixelPoint(), bottomRight->pixelPoint() );
  switch ( anchorId ) {
  case aiTopLeftRim:     return rect.center() + ( rect.topLeft() - rect.center() )
                                  * 1 / qSqrt( 2 );
  case aiTop:            return ( rect.topLeft() + rect.topRight() ) * 0.5;
  case aiTopRightRim:    return rect.center() + ( rect.topRight() -
                                  rect.center() ) * 1 / qSqrt( 2 );
  case aiRight:          return ( rect.topRight() + rect.bottomRight() ) * 0.5;
  case aiBottomRightRim: return rect.center() + ( rect.bottomRight() -
                                  rect.center() ) * 1 / qSqrt( 2 );
  case aiBottom:         return ( rect.bottomLeft() + rect.bottomRight() ) * 0.5;
  case aiBottomLeftRim:  return rect.center() + ( rect.bottomLeft() -
                                  rect.center() ) * 1 / qSqrt( 2 );
  case aiLeft:           return ( rect.topLeft() + rect.bottomLeft() ) * 0.5;
                                  ...
                                  QVariant *details ) const
{
  ...
  case tsPlus: {
    if ( clipRect().intersects( QRectF( center - QPointF( w, w ),
                                        center + QPointF( w, w ) ).toRect() ) )
...
void QCPItemTracer::draw( QCPPainter *painter )
{
  ...
  case tsPlus: {
    if ( clip.intersects( QRectF( center - QPointF( w, w ), center + QPointF( w,
                                  w ) ).toRect() ) ) {
      painter->drawLine( QLineF( center + QPointF( -w, 0 ), center + QPointF( w,
                                 0 ) ) );
      painter->drawLine( QLineF( center + QPointF( 0, -w ), center + QPointF( 0,
                                 w ) ) );
    }
    break;
  }
  case tsCrosshair: {
    if ( center.y() > clip.top() && center.y() < clip.bottom() )
      painter->drawLine( QLineF( clip.left(), center.y(), clip.right(),
                                 center.y() ) );
    if ( center.x() > clip.left() && center.x() < clip.right() )
      painter->drawLine( QLineF( center.x(), clip.top(), center.x(),
                                 clip.bottom() ) );
    break;
  }
  case tsCircle: {
  ...
  case tsSquare: {
    if ( clip.intersects( QRectF( center - QPointF( w, w ), center + QPointF( w,
                                  w ) ).toRect() ) )
      painter->drawRect( QRectF( center - QPointF( w, w ), center + QPointF( w,
                                 w ) ) );
    break;
  }
  }
        ...
        if ( mGraphKey < first.key() )
          ...
          if ( it != first ) { // mGraphKey is somewhere between iterators
            ...
            if ( mInterpolating ) {
              ...
              double slope = ( it.value().value - prevIt.value().value ) /
                             ( it.key() - prevIt.key() );
                                   ...
                                   QVariant *details ) const
{
  ...
  return qSqrt( distSqrToLine( ( centerVec - widthVec ).toPointF(),
...
void QCPItemBracket::draw( QCPPainter *painter )
{
  ...
  if ( clip.intersects( boundingPoly.boundingRect() ) ) {
    ...
    case bsCurly: {
      ...
      QPainterPath path;
      path.moveTo( ( centerVec + widthVec + lengthVec ).toPointF() );
      path.cubicTo( ( centerVec + widthVec * 1 - lengthVec * 0.8 ).toPointF(),
                    ( centerVec + 0.4 * widthVec + 1 * lengthVec ).toPointF(),
                    centerVec.toPointF() );
      path.cubicTo( ( centerVec - 0.4 * widthVec + 1 * lengthVec ).toPointF(),
                    ( centerVec - widthVec * 1 - lengthVec * 0.8 ).toPointF(),
                    ( centerVec - widthVec + lengthVec ).toPointF() );
      painter->drawPath( path );
      break;
    }
    case bsCalligraphic: {
      ...
      path.cubicTo( ( centerVec - widthVec * 1 - lengthVec * 0.5 ).toPointF(),
      ...
      path.cubicTo( ( centerVec + 0.2 * widthVec + 1.2 * lengthVec ).toPointF(),
                    ( centerVec + widthVec * 1 - lengthVec * 0.5 ).toPointF(),
                    ( centerVec + widthVec + lengthVec ).toPointF() );
...
QCPAxis *QCPAxisRect::addAxis( QCPAxis::AxisType type )
{
  ...
  if ( mAxes[type].size() >
       0 ) { // multiple axes on one side, add half-bar axis ending to additional axes with offset
    ...
    if ( mBackgroundScaled ) {
      ...
      if ( mScaledBackgroundPixmap.size() != scaledSize )
        mScaledBackgroundPixmap = mBackgroundPixmap.scaled( mRect.size(),
                                  mBackgroundScaledMode, Qt::SmoothTransformation );
      painter->drawPixmap( mRect.topLeft(), mScaledBackgroundPixmap, QRect( 0, 0,
                           mRect.width(), mRect.height() ) & mScaledBackgroundPixmap.rect() );
    } else {
      painter->drawPixmap( mRect.topLeft(), mBackgroundPixmap, QRect( 0, 0,
...
void QCPAxisRect::updateAxesOffset( QCPAxis::AxisType type )
{
  ...
  for ( int i = 1; i < axesList.size(); ++i )
    axesList.at( i )->setOffset( axesList.at( i - 1 )->offset() + axesList.at(
                                   i - 1 )->calculateMargin() + axesList.at( i )->tickLengthIn() );
}
...
{
  mDragStart =
    event->pos(); // need this even when not LeftButton is pressed, to determine in releaseEvent whether it was a full click (no position change between press and release)
...
void QCPLegend::setSelectedParts( const SelectableParts &selected )
{
  ...
  mSelectedParts =
    this->selectedParts(); // update mSelectedParts in case item selection changed
...
QSize QCPPlotTitle::minimumSizeHint() const
{
  ...
  QSize result = metrics.boundingRect( 0, 0, 0, 0, Qt::AlignCenter,
                                       mText ).size();
